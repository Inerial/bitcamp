...
from keras.layers import ZeroPadding2D, Conv2D, BatchNormalization, LeakyReLU, MaxPooling2D, concatenate

# activation
def leaky_relu(x):
#    return tf.nn.leaky_relu(x,alpha=0.1,name='leaky_relu') # 혹 tf.maximum(0.1*x,x)
   return LeakyReLU(alpha=0.1)(x)

# Conv+BN: yolo2중 매 CNN층 뒤에 한개의 BN층이 붙음
...
def conv2d(x,filters_num,filters_size,pad_size=0,stride=1,batch_normalize=True,
        ...
        # padding 주의: padding = "same"을 사용안하면 좌표 계산 착오가 발생할 수 있음

    if pad_size > 0:
        #   x = tf.pad(x,[[0,0],[pad_size,pad_size],[pad_size,pad_size],[0,0]])
        x = ZeroPadding2D(pad_size)(x)
    ...
    out = Conv2D(filters=filters_num,kernel_size=filters_size,strides=stride,padding='valid',activation=None,use_bias=use_bias)
...
#    return tf.layers.max_pooling2d(x,pool_size=size,strides=stride)
   return MaxPooling2D(pool_size=size,strides=stride)(x)
...
def darknet(images,n_last_channels=425):
   ...
   net = concatenate([shortcut, net], axis=-1) # channel整合到一起
