...
for i in range(int(x.shape[0]/time_step)):
    mid = x.iloc[i*time_step : i*time_step+200, 1:].values
    # for j in range(4):
    ...
    mid = np.concatenate([mid, x.iloc[i*time_step : i*time_step+200, 0:1].values], axis = 1)
    tmp.append(mid)
x_LSTM = np.array(tmp)

tmp = []
for i in range(int(x_pred.shape[0]/time_step)):
    mid = x_pred.iloc[i*time_step : i*time_step+200, 1:].values
    ...
    #for j in range(4):
    #     mid = np.concatenate([mid, np.fft.fft(scaler.fit_transform(mid[:,j:j+1])[0], n=375*2).real[:375].reshape(375,1), np.fft.fft(scaler.fit_transform(mid[:,j:j+1])[0], n=375*2).imag[:375].reshape(375,1)], axis = 1)
    mid = np.concatenate([mid, x_pred.iloc[i*time_step : i*time_step+200, 0:1].values], axis = 1)
    tmp.append(mid)
