...
model.compile(optimizer=Adam(0.002), loss='categorical_crossentropy', metrics=['acc'])

check = ModelCheckpoint('./dacon/comp7/bestcheck.hdf5', monitor='val_loss',save_best_only=True)
model.fit(x_train,y_train,batch_size=64, epochs=100, validation_split=0.2, callbacks=[check])
