...
for i in range(len(x_real)):
    mkchannel1 = []
    for j in range(26):
        if x_real_letter[i,j] is 0:
            # mkchannel.append(x_real[i]/25)
            mkchannel1.append(np.zeros(x_real[i].shape))
        else:
            mkchannel1.append(x_real[i])
    x_real_channel.append(list(cv2.merge(mkchannel1)))
...
input1 = Input(shape=(28,28,26))

conv1 = Conv2D(64,(3,3), kernel_regularizer=regul)(input1)
...
reduction = ReduceLROnPlateau(monitor='acc', patience=3, verbose=1, factor=0.5, min_lr=0.00001)
...
check = ModelCheckpoint('./dacon/comp7/bestcheck.hdf5', monitor='acc',save_best_only=True)
